...
export interface AnchoredPosition {
  ...
  maxHeight: number
  /**
   * Carried here rather than read from `window` at render, so the details card
   * — which picks its side from the room left over — recomputes on resize off
   * the same measurement pass as the panel, instead of holding a width from
...
export function useAnchoredPosition(
  ...
  React.useEffect(() => {
    ...
    measure()
    // Measuring once is not enough: the trigger can still be settling as the
    // panel opens, leaving it narrower than its own trigger until something
    // else forces a re-measure.
